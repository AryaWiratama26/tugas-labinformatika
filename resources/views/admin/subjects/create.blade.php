...
<div class="max-w-2xl bg-white rounded-3xl border border-black/5 shadow-sm p-8">
    <form action="{{ route('admin.subjects.store') }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <input type="text" name="code" value="{{ old('code') }}" class="w-full px-4 py-3 rounded-2xl bg-white border border-black/10 focus:border-[#4A5D4E] focus:outline-none focus:ring-4 focus:ring-[#4A5D4E]/10 transition-all">
